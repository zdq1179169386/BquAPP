...
- (void)customInit{
    ...
    //退到后台再进入app，动画会结束，
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveNotify) name:UIApplicationDidBecomeActiveNotification object:nil];
    ...
    self.label = label;
//    label.center = CGPointMake(ScreenWidth/2.0, rect.size.height+25);
...
- (void)startMoveLine{
    ...
    self.label.center = CGPointMake(ScreenWidth/2.0, self.scanAreaRect.origin.y - 25);
